...
- **Restore through git, never through a scratch copy.** Mutation testing deliberately edits *working* code, so the restore path must be as trustworthy as the mutation. A tool (`cargo mutants`, `mutmut`) handles this for you; a **hand-run** mutation does not, and `cp file /tmp/f.bak` is the trap — a stale backup from an earlier refactor will silently restore the *wrong* version, and you have now "restored" a regression into a green suite. Snapshot with `git stash` / restore with `git checkout <path>`, and re-run the full suite after every restore, not just the mutated test. Observed cost: a `/tmp` backup taken before an ownership refactor clobbered the refactored module; only a compile error caught it, and a version that still compiled would have shipped.
